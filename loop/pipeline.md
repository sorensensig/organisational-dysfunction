...
- If any: pick the target branch. **If kihub's initial contribution PR is still open** (the skill isn't on kihub `main` yet), check out *that* PR's branch and add to it — do **not** open a second PR. Otherwise branch off kihub `main`. Then **copy only the new `references/NN-slug.md` files** into kihub's skill folder and sync kihub's `SKILL.md` with source (the router carries no repo-specific content). Run `npm ci` (once), `npm run skill:validate`, and `npm run build` (regenerates `docs/README.skills.md`).
...
- Commit and push. If you added to an existing open PR it now reflects the additions; otherwise open a new PR to `Altinn/kihub` describing which dysfunctions were added and crediting the source series. **Then verify** the remote PR's changed-file list actually includes the new reference files before reporting success.
